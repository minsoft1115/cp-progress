...
- **정지는 즉시여야 한다 — 틱 사이의 대기는 깨울 수 있어야 한다.** 플래그를 루프 맨 위에서만
  읽고 아래에서 그냥 자면, teardown이 플래그를 세워도 join은 진행 중인 sleep이 끝나기를 기다린다.
  그러면 **모든 managed 복사가 `cp`보다 최대 한 샘플 주기만큼 오래 산다** — 기본 100ms에서는
  눈에 안 띄지만 작은 복사에서는 이게 벽시계 오버헤드의 지배 항이다(`performance.md`).
  그래서 대기는 조건변수이고, `stop`은 플래그를 세운 뒤 깨운다.
