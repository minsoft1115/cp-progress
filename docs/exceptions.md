...
| D10 | <a id="d10"></a>**footer가 떠 있는 동안 도착한 여러 조각짜리 메시지가 화면에서 유실** | **#76으로 규칙이 사라졌다.** 예전에는 터미널에 개행으로 끝나지 않은 줄이 남아 있는 동안 footer를 보류했다 — footer가 로그와 같은 흐름에 있어 그 줄을 덮어썼고, glibc `error()`가 한 줄을 write 4회로 내므로 보류하지 않으면 개행을 품은 마지막 조각만 살아남았다. footer가 스크롤 영역 밖 절대 행으로 옮겨간 뒤로는 미완성 로그 줄에 닿을 수가 없다([ui.md](./ui.md) 불변식 11이 6a로 대체된 것과 같은 사유) | ✅ 이제 이 예외를 막는 것은 보류 규칙이 아니라 배치다 — cprog가 로그 바이트에 손을 대지 않는다는 것이 곧 조각이 유실되지 않는다는 뜻이고, 그것을 고정하는 것은 `render.rs::log_bytes_are_written_straight_through`다([F9](#f9)와 같은 근거). 규칙을 담고 있던 상태 필드와 그것을 잡던 유닛 다섯은 규칙과 함께 삭제됐고, 그 시절의 실제 사례는 [ui.md](./ui.md) "왜 11번이 필요한가"에 남겨 뒀다 |
...
| F7 | <a id="f7"></a>**`SIGKILL` / `SIGSEGV`** | 핸들러도 `Drop`도 못 돈다 → **footer 잔상 + 커서가 숨겨진 채로, 그리고 스크롤 영역이 걸린 채로 터미널이 남는다.** `PDEATHSIG`로 cp는 정리되지만 화면은 사용자가 `printf '\033[r'; tput cnorm` / `reset`으로 복구해야 함 | 📄 회피 불가 — `usage.md`의 `printf '\033[r'; tput cnorm` 안내 (#10) |
...
| F9 | <a id="f9"></a>**로그 바이트 도착** | **그대로 통과시킨다.** #76 전에는 footer 지움 → 로그 씀 → footer 재그림(erase-redraw)이었으나, footer가 스크롤 영역 밖으로 나가면서 둘이 물리적으로 겹치지 않게 됐다 — 로그는 영역 안 커서 자리에 떨어지고 터미널이 그 영역을 스크롤한다. 지울 것도 되돌릴 것도 없다([ui.md](./ui.md) 불변식 6a, [testing.md](./testing.md) C8) | ✅ `render.rs::log_bytes_are_written_straight_through` |
...
| F14 | <a id="f14"></a>**크기 조회 실패 / 폭 0** (`TIOCGWINSZ`가 에러이거나 `ws_col == 0` — 크기가 설정된 적 없는 pty가 대표적) | **footer를 그리지 않는다.** 복사 자체는 그대로 진행되고 `cp`의 출력도 그대로 흐른다 — 진행바만 없다. 조회가 한 번이라도 성공하면 그때부터 그리고, 이후 실패는 마지막 성공값을 유지한다. **#76 전에는 80×24를 지어내고 그렸다.** 그때는 배치에 닿는 것이 폭뿐이라 감당 가능한 거짓말이었지만, footer가 스크롤 영역 안 절대 행으로 옮겨간 뒤로는 높이가 **위치를 결정**한다 — 지어낸 높이는 영역과 바를 화면 바닥이 아닌 행에 박고, 로그는 엉뚱한 영역에서 스크롤한다. `apt`도 같은 이유로 크기를 모르면 fancy progress를 안 켠다(`install-progress.cc`: `SetupTerminalScrollArea`가 `nr_rows <= 1`에서, `DrawStatusLine`이 `size.rows < 1`에서 즉시 반환). `plan.rs`의 모든 능력 검사가 실패하면 passthrough로 가는 것과 같은 원칙이다 | ✅ `term::terminal_size`, `tests/resize.rs::an_unsized_terminal_gets_no_footer_at_all` (#50, #51, #76) |
...
| F20 | <a id="f20"></a>**뮤테이션 생존자 판정 대장** | 전수 뮤테이션(`cargo mutants -j 3 -- --features integration`)에서 살아남지만 **테스트 공백이 아닌** 것들. 다시 파생하지 않도록 판정과 근거를 적어둔다. 모두 실측이다(변이를 걸고 통합 스위트를 돌린 결과). **① equivalent — 프로그램이 같아 관측 차이가 없다:** `ui::render_footer`의 `MIN_LOG_ROWS + FOOTER_ROWS` → `*`(둘 다 2라 `2+2 == 2*2`); `render::FooterGuard::draw`의 같은 식 → `*`(같은 이유, #76에서 footer 억제 판정이 렌더러 쪽에도 생기며 두 번째 사본이 됐다 — 같은 줄의 나머지 네 변이 `||`→`&&`·`<`→`<=`·`<`→`==`·`+`→`-`는 `render.rs::a_terminal_too_short_for_a_footer_gets_nothing_at_all`이 전부 죽인다); `ui::name_row`의 `cols == 0 || clean.is_empty()` → `&&`(빈 이름은 폭 검사를 통과해 그대로 반환되고 — 빈 문자열이라 `String::new()`와 같은 값 — 폭 0에 비지 않은 이름은 말줄임표 자리조차 없어 `checked_sub`가 `None`을 준다. **"어긋난 갈래도 결국 `String::new()`에 닿는다"까지는 참이 아니다**: `cols == 0`인데 이름이 폭 0이면서 비어 있지 않으면(결합 문자·ZWJ만으로 된 이름) 뮤턴트는 조기 반환을 건너뛰고 `0 <= 0`을 통과해 **그 바이트를 그대로 돌려준다**. 판정이 equivalent로 남는 근거는 그 입력이 아니라 **`cols == 0`이 도달 불가**라는 것이다 — 크기를 못 읽은 터미널도 초기값 80을 유지한다([F14](#f14), #69 D)); `ui::compose`의 `fixed_width + sep_total` → `-`(이 갈래에 닿는 단계가 percent 하나뿐이라 `sep_total == 0`); `lib.rs`의 `received != 0 && …` → `||`(`Signal::from_named_raw(0)`이 `None`이라 `if let`이 kill을 건너뛴다); `lib.rs`의 로그 도착 갈래 `progress_shown |= footer.is_some()` → `&=`(**이후** tick의 타임아웃 갈래가 다시 세운다 — 둘은 같은 `match received`의 배타적 arm이라 한 tick에 하나만 돌므로 "같은 tick"이 아니다, #69 D. **두 자리를 함께 바꾸면 잡히므로** 한 자리씩 보는 뮤테이션에서만 생존한다). **성격이 다른 equivalent 하나** — `process::exec_replace`의 exec 직전 `SIGPIPE → SIG_DFL` 복원: 지워도 스위트가 그대로 통과한다. 우리 코드가 같아서가 아니라 **Rust std의 `CommandExt::exec`가 spawn과 같은 prologue를 돌며 SIGPIPE를 이미 리셋하기 때문**이고, 그래서 뮤턴트도 순정 `cp`처럼 SIGPIPE로 죽어 `tests/passthrough.rs::verbose_to_a_closed_pipe_dies_of_sigpipe_like_cp`가 통과한다(실측). **그래도 코드는 남긴다** — 계약을 std의 내부 동작에 맡기는 건 [B10](#b10)이 기록한 종류의 위험이다(의존 대상이 *값*으로 표현하던 것을 다음 버전이 *에러*로 표현하면 조용히 깨진다). 즉 "테스트 공백"이 아니라 "현재 std에서 equivalent"이며, A7의 근거 테스트는 결과(순정과 같은 시그널 사망)를 고정하지 메커니즘을 고정하지 않는다(#59). **② 도달 불가 — 갈래는 다르나 실행이 안 닿는다:** `ui::render_bar` ASCII 갈래의 `width < 2`(`bar_cells`가 10 이상만 준다); `proc::source_for`의 `*fd < dest_fd` → `<=`(`dest_fd`는 쓰기 fd라 `sources`에 없다). **③ 이 플랫폼에서 관측 불가:** `term::proc_available`의 `cfg!(linux) && /proc 존재` → `||`/`true`(리눅스+procfs에선 두 항이 다 참이라 구분되지 않는다. 비-리눅스나 procfs 없는 호스트라야 관측된다) | 📄 판정 기준은 [`testing.md`](./testing.md) "가드레일" (#53) |
| F21 | <a id="f21"></a>**창을 넓힐 때 남는 옛 footer 조각** | **수용한다.** 넓히면 터미널이 스크롤백에서 줄을 끌어내려 화면을 채우고, 이전에 그려진 footer의 조각이 로그 커서보다 **위**로 돌아올 수 있다. cprog가 지우는 범위는 커서부터 아래(`ESC[J`)이므로 닿지 않는다. **닿는 방법은 `ESC[2J`(보이는 화면 전체) 하나뿐이고, 넣었다가 되돌렸다** — 잔상과 함께 셸 프롬프트·사용자가 친 명령줄·화면의 `cp -v` 로그를 지우고, cprog는 그 바이트를 본 적이 없어 복원할 수 없다(실제 터미널에서 한 번만 넓혀도 명령을 실행한 줄이 사라지는 것이 관측됐다). 대안도 없다: 리플로 뒤에는 무엇이 몇 행 움직였는지 계산할 수 없고(#76의 실패한 시도 3개), 화면을 읽어 확인하는 것도 불가능하다 — DSR·DECRQCRA의 응답이 터미널의 **입력**으로 오는데 stdin은 `cp`가 물려받은 사용자 것이다(`process.rs`의 `stdin stays inherited`). 잔상은 로그가 흘러가면 밀려 사라지며, 사용자가 잃는 것은 없다 | 📄 한계를 명시하는 테스트로 고정: `render.rs::a_widening_leaves_its_residue_above_the_cursor_and_that_is_accepted`, 되돌린 쪽의 가드는 `no_resize_direction_blanks_what_is_already_on_screen` (#76) |
