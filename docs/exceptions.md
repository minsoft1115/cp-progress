...
| F8 | **렌더/IO 실패** | best-effort. exit code 불변 | ✅ `render.rs::io_failure_is_returned_and_drop_never_panics`(실패가 `Err`로 표면화되고 `Drop`이 panic하지 않는 것)**과** `tests/exit_contract.rs`(그 실패가 exit code를 안 바꾸는 것). 앞의 테스트만으로는 exit code를 한 번도 안 보므로 이 행의 절반만 근거가 된다 — D7과 같은 짝이다 |
