...
| A1a | **`s`가 실시간 시그널**(`SIGRTMIN`~`SIGRTMAX`) | A1과 **동일하게** signaled로 재현한다. 재현을 맡은 `signal_hook::low_level::emulate_default_handler`의 표는 표준 시그널까지만 담고 있어 이 구간은 `EINVAL`을 돌려주므로, 그때는 `low_level::raise`로 직접 올린다 — cprog는 SIGINT/TERM/HUP/QUIT/WINCH/TSTP에만 핸들러를 달므로 이 경로에 오는 시그널은 이미 기본 disposition에 unblock 상태이고, raise 하나가 곧 재현 전체다. 이 폴백이 없으면 `128 + s`로 **정상 종료**해 A1이 깨진다 | ✅ `tests/signals.rs::cp_killed_by_a_realtime_signal_still_exits_cprog_signaled` (#43) |
