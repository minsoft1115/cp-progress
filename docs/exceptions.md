...
| F14 | **파일명 관련 렌더 사고** | `-v` 없이 실행하면 footer 1행이 대상 경로를 표시하므로 **다시 표면이 생겼다**(#20). 제어문자를 제거한 뒤 표시폭 기준으로 앞에서 자르며, footer가 2행이 되면서 폭 초과는 미관이 아니라 **정확성** 문제가 되었다(줄이 접히면 커서 이동이 어긋남) | ✅ **해결(#20)** — sanitize + 폭 절단, `ui.md` 불변식 7 |
