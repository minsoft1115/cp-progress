...
|---|---|---|---|---|---|
| 진짜 `cp`인가? | ✓ 진짜 cp를 밖에서 관찰 | △ GNU cp + 진행바 패치(직접 빌드) | ✗ 자체 구현(Rust) | ✗ 다른 도구 | ✓ 진짜 cp를 감쌈 |
| 어떻게 실행하나 | 복사 옆에서 명령 하나 더(`progress -mp $!` / `watch progress`) | `advcp -g …`(또는 alias로 `cp` 대체) | `cpx …`(자체 명령) | `rsync -a --info=progress2 …` | `cprog <cp 인자…>` — 인자가 cp 그대로; installer가 `cp`를 alias로 대체 |
| 설치 | 배포판 패키지 | 패치 + coreutils 재컴파일 | `cargo install` / AUR | 대개 기본 설치 | `cargo install`/한 줄 |
| 최신 coreutils 추종 | ✓ 포팅할 것 없음 — 돌고 있는 cp를 관찰 | 패치가 겨냥한 coreutils 릴리스에 고정 | 해당없음(자체 코드) | 해당없음 | ✓ 시스템 cp에 얹혀 항상 최신 |
| cp 동작 바뀔 위험 | 없음 | 낮음 — GNU cp + 패치 하나 | 있음 — 독립 구현이므로 | 설계상 의미론이 다름 | 없음 — 진짜 cp를 그대로 실행 |
| 진행바 정확도 | 밖에서 추정(fd 위치 — 모든 복사 경로가 갱신하진 않음) | 높음 | 높음 | 높음 | 근사·per-file |

표 요약 — 서로 다른 일에 맞는 서로 다른 도구다:

- **`progress`** — 철학이 가장 가깝다: 진짜 cp를 밖에서 지켜보고, 복사 경로에 아무것도 심지
  않는다. 대가는 명령을 하나 더 띄워야 한다는 것, 그리고 바깥 관찰이라 커널이 노출하는 것만
  읽을 수 있다는 것 — 모든 복사 경로가 그 값을 갱신하지는 않는다. (cprog는 이 관찰자
  아이디어를 이어받되, 대상 파일의 커지는 크기를 읽는다 —
  [`docs/progress-model.md`](./docs/progress-model.md).)
- **`advcpmv`** — `cp` 자신이 낼 수 있는 가장 정확한 바다. GNU cp에 진행바 패치를 얹은 것
  그 자체이기 때문이다. 대가는 coreutils를 직접 빌드해야 하고, 패치가 겨냥한 릴리스에
  머문다는 것.
- **`cpx`** — 속도를 명시적 목표로 삼은, 바가 잘 다듬어진 현대적 재구현. 대가는 자체
  도구라는 것 — `cp`의 정확한 의미론이 계약의 일부가 아니다.
- **`rsync`** — 강력하고 어디에나 있으며, 동기화에는 정답이다. 다만 `cp`와는 다른 질문에
  답하는 도구라, 인자와 보존 의미론도 자기 것이다.
